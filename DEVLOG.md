...
### 삼성전자 제4호(허은녕) 파싱 실패 수정

**문제**: `agm_personnel`에서 삼성전자 제4호 "감사위원회 위원이 되는 사외이사 허은녕 선임의 건"이 누락됨
**원인**: HTML에서 `<SPAN>제4호 의안) </SPAN><SPAN>감사위원회 위원이 되는...</SPAN>` 처럼 제목이 두 SPAN으로 분리됨. `_split_p_lines()`의 줄바꿈 합침 정규식이 `[■□●▶]` 접두 기호를 필수로 요구해서, 기호 없이 시작하는 제목의 줄바꿈이 합쳐지지 않음. title이 `)`만 남아 "선임" 키워드 매치 실패.
**수정**: 접두 기호를 선택적(`?`)으로 변경, `의안/안건` 뒤 `)）` 패턴 추가
**검증**: 삼성전자 제4호 허은녕 정상 추출, KT&G regression 통과
